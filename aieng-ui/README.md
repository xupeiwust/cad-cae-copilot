...
- **Shape IR conversion (multi-target)** — `.shape.json` / `.shape_ir.json` sources convert into `.aieng` and compile by `representation` through a pluggable compiler registry: `brep_build123d` (default) → build123d → STEP/B-Rep; `implicit_sdf` → fogleman/sdf marching-cubes mesh; `manifold_mesh` → manifold3d CSG mesh. Shape IR is the source; STEP/STL/GLB are derived products recorded at different evidence levels in `provenance/conversion_manifest.json`
...
| `aieng.convert` | Working — imports STEP/STP/FCStd/Shape IR. Shape IR compiles by `representation` (pluggable registry): `brep_build123d` → build123d STEP/B-Rep; `implicit_sdf` → SDF marching-cubes mesh; `manifold_mesh` → manifold3d CSG mesh (mesh reps → STL/GLB, region-level faces). Publishes a viewer preview |
