...
- **Shape IR conversion (multi-target)** — `.shape.json` / `.shape_ir.json` sources convert into `.aieng` and compile by `representation`: `brep_build123d` (default) → build123d `source.py` → STEP/B-Rep; `implicit_sdf` → `sdf_source.py` → fogleman/sdf marching-cubes mesh. Shape IR is the source; STEP/STL/GLB are derived products recorded at different evidence levels in `provenance/conversion_manifest.json`
...
| `aieng.convert` | Working — imports STEP/STP/FCStd/Shape IR. Shape IR compiles by `representation`: `brep_build123d` executes build123d `source.py` → STEP/B-Rep; `implicit_sdf` runs the SDF runner (`sdf_source.py` → marching-cubes mesh → STL/GLB, region-level faces). Publishes a viewer preview |
