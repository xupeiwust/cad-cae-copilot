        ...
        "properties": {
            ...
            "range": {
                ...
                "properties": {
                    ...
                    "steps": {"type": "integer", "minimum": 2, "maximum": 25, "description": "Number of evenly spaced values to generate."},
            ...
            "apply_winner": {"type": "boolean", "default": False, "description": "If true, apply the recommended value to the baseline through the audited cad.edit_parameter path (still one approval; reports regression_diff)."},
        },
        "additionalProperties": False,
        "description": (
            "[APPROVAL REQUIRED] Parametric sizing sweep: vary ONE editable dimension across values "
            "or a {min,max,steps/step} range, solve EACH variant with the real static solver (Gmsh + CalculiX), "
            "and rank by objective subject to a stress/displacement constraint. Baseline is never "
            "modified unless apply_winner=true, in which case the winning value is applied through the "
            "approval-gated cad.edit_parameter path. Runs N solver executions."
        ),
    },
