...
| `cad.validate_targets` | Deterministic geometry **target validator**: pass a list of targets (`named_part_present`, `feature_present`, `part_count`, `overall_size`, `part_size`, `part_center`, `no_floating_parts`, `no_deep_overlap`) and get pass/fail/unknown per target with measured-vs-expected. Verifies the modeling brief's exact promises — catches plausible-but-mispositioned / over-modeled builds. Bbox-level, not GD&T; read-only |
