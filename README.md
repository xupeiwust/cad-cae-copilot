...
- **Assembly CAE + topopt setup v0:** optional Assembly IR packages can produce a solver-neutral simplified proxy CAE model, optional simplified CalculiX deck when real mesh refs exist, honest skipped solver diagnostics, and assembly result maps back to parts/interfaces/connections. A backend-only assembly-aware topology-optimization setup can then derive a single selected design-part problem (`analysis/assembly_topopt_problem.json`, `diagnostics/assembly_topopt_derivation.json`, and only when safe `analysis/topology_optimization_problem.json`) with interface preserve regions and advisory result guidance. It does not model nonlinear contact, bolt preload, or simultaneous multi-part optimization and is not production certified.
