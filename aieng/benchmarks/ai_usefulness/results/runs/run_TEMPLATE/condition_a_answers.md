...
**TEMPLATE — replace all FILL_IN markers with actual run data. Do not score here.**
...
| Field | Value |
|-------|-------|
| run_id | FILL_IN (e.g. `run_20260601T143022Z`) |
| model | FILL_IN (e.g. `claude-sonnet-4-6`) |
| provider | FILL_IN (e.g. Anthropic) |
| session date (UTC) | FILL_IN |
| temperature | FILL_IN (e.g. `1.0` / `default` / `unknown`) |
| system prompt | FILL_IN (`none` / paste verbatim if used) |
...
FILL_IN — paste the full verbatim AI response here
...
FILL_IN — any anomalies, interruptions, or unexpected behaviors during this session
(or "none" if the session ran normally)
