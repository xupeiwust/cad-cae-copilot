...
def test_pareto_demo_end_to_end(tmp_path: Path) -> None:
    ...
    assert rank_res["candidate_count"] == 4
    ...
    assert set(ranking["pareto_front"]["front_candidate_ids"]) == {"c_light", "c_heavy"}
    ...
    assert {item["candidate_id"] for item in pareto_artifact["front"]} == {"c_light", "c_heavy"}
    ...
    assert report_res["candidate_count"] == 4
    ...
    assert set(report["pareto_front"]["front_candidate_ids"]) == {"c_light", "c_heavy"}
