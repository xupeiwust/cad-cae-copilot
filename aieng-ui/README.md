...
- **Assembly CAE v0** — Assembly IR packages can produce `simulation/assembly_cae_model.json`, simplified proxy connection diagnostics, optional `simulation/assembly_calculix.inp` only when mesh prerequisites exist, best-effort/skipped solver execution diagnostics, and `analysis/assembly_result_map.json`. Connections remain simplified proxies; no nonlinear contact or bolt preload is claimed.
