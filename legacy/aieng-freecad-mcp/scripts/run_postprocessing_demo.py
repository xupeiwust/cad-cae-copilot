"""Post-processing evidence demo.

Runs the CAD-to-CAE workflow with post-processing enabled using:
...
async def main() -> int:
    print("=" * 60)
    print(" Post-Processing Evidence Demo")
    print("=" * 60)
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        print("\n3. Running CAD-to-CAE workflow with post-processing...")
        ...
        print(f"\n6. Post-processing")
        if summary.postprocess_summary:
            pp = summary.postprocess_summary
            print(f"   Status: {pp['status']}")
            print(f"   Metrics: {len(pp.get('metrics', []))}")
            for metric in pp.get("metrics", [])[:5]:
                status = metric.get("status", "unknown")
                name = metric.get("name", "unknown")
                value = metric.get("value", "N/A")
                print(f"      {name}: {value} ({status})")
            for artifact in pp.get("artifacts_written", []):
                print(f"   Artifact: {Path(artifact['path']).name} ({artifact['artifact_type']})")
        else:
            print("   No post-processing summary present.")
        ...
        # Verify post-process evidence metadata
        if evidence["entries"]:
            for entry in reversed(evidence["entries"]):
                meta = entry.get("metadata", {})
                if meta.get("engineering_validation") is False:
                    print(f"\n8. Post-process discipline verified")
