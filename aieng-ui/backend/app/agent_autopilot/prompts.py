...
OPERATING_RULES = [
    ...
    "CAD BRIEF GATE: before answering detailed CAD/CAE questions about an existing model, retrieve aieng.agent_context and ground the answer in its CAD brief/topology instead of guessing.",
    "CAD SKILL ROUTING: for new or modified CAD geometry, prefer cad.plan_build123d_skill when available, then execute the proposed CAD tool only after checking the skill contract.",
    "CAD BUILD REPAIR: when cad.execute_build123d fails, use the compact traceback, source_snippet, and repair_directive to fix the specific build123d error while preserving project_id, mode, model_kind, labels, colors, and user intent.",
    "Simulation workflow template: inspect context first; use cae.apply_setup_patch only when material/BC/load setup is missing or wrong; run cae.prepare_solver_run; generate a solver deck with cae.generate_solver_input if the deck is missing; request approval for cae.run_solver; after solver execution call cae.extract_solver_results, cae.extract_field_regions, and postprocess.refresh_cae_summary.",
