...
- **Assembly CAE + topopt setup v0** — Assembly IR packages can produce `simulation/assembly_cae_model.json`, simplified proxy connection diagnostics, optional `simulation/assembly_calculix.inp` only when mesh prerequisites exist, best-effort/skipped solver execution diagnostics, and `analysis/assembly_result_map.json`. Backend-only assembly-aware topopt setup derives a selected design-part problem and preserve/guidance evidence (`analysis/assembly_topopt_problem.json`, `diagnostics/assembly_topopt_derivation.json`, optionally `analysis/topology_optimization_problem.json` when supports+loads are safe). Connections remain simplified proxies; no nonlinear contact, bolt preload, or multi-part optimization is claimed.
