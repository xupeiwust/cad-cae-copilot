...
| ID | 任务名称 | 当前状态 | 优先级 | 关联文件 | 说明 | 验收标准 | 备注 |
|---|---|---|---|---|---|---|---|
| WCA-P5-001 | Plan replay 联调测试 | TODO | P0 | `chatTranscript.ts`, `test_api.py`, 前端手测 | 刷新页面后 Plan 事件和 run snapshot 应一致。 | refresh 后 plan/step 状态不丢失。 | 现有能力需补回归测试。 |
| WCA-P5-002 | context summary UI/API 联调 | TODO | P0 | P3/P4 相关文件 | 摘要刷新后 UI 立即更新。 | 长会话能显示最新摘要。 | 依赖 P3/P4。 |
| WCA-P5-003 | approval mode 联调 | TODO | P1 | `db.py`, `api.ts`, `ChatPanel.tsx` | 审批模式影响后端 policy 执行。 | `strict/balanced/manual` 三档可验证；高风险 CAD/CAE mutation 与 solver 不受自动放行影响。 | 已采纳默认策略。 |
| WCA-P5-004 | SSE + polling fallback 回归 | TODO | P1 | `useAgentActivityStream.ts`, `agent_activity.py` | live stream 断开时仍能恢复项目与 run 状态。 | 手测和测试覆盖 reconnect/polling。 | 现有基础已存在。 |
...
| ID | 任务名称 | 当前状态 | 优先级 | 关联文件 | 说明 | 验收标准 | 备注 |
|---|---|---|---|---|---|---|---|
| WCA-P6-001 | CAD/CAE context source 面板 | TODO | P2 | `useGeometryPointers.ts`, `ContextSummaryPanel` | 展示当前项目、选中面、关键 artifact、CAE readiness。 | Agent 可解释当前使用了哪些工程上下文。 | 依赖 context summary。 |
| WCA-P6-002 | 仿真任务 Plan 模板 | TODO | P2 | `intent_planner.py`, `agent_autopilot/prompts.py` | 将 run simulation 拆解为检查、预处理、审批、执行、解析。 | Plan steps 与现有 CAE 工具匹配。 | 保持审批边界。 |
| WCA-P6-003 | CAD 修改任务恢复机制 | TODO | P2 | `engine.py`, `context_memory.py` | 修改失败时能基于摘要和错误继续 repair。 | CAD build error 有可读修复路径。 | 已有 repair loop，可增强场景化。 |
...
| WCA-P5-001 | Phase 5 | Plan replay 联调测试 | TODO | P0 | `chatTranscript.ts`, `test_api.py` | 刷新后 Plan 状态不丢 | 现有能力需回归 |
| WCA-P5-002 | Phase 5 | context summary UI/API 联调 | TODO | P0 | `ContextSummaryPanel`, context summary API | 摘要刷新后 UI 更新 | 依赖 P3/P4 |
| WCA-P5-003 | Phase 5 | approval mode 联调 | TODO | P1 | `db.py`, `api.ts`, `ChatPanel.tsx` | 审批策略影响后端执行 | 三档策略联调 |
| WCA-P5-004 | Phase 5 | SSE + polling fallback 回归 | TODO | P1 | `useAgentActivityStream.ts`, `agent_activity.py` | 断流后可恢复状态 | 现有基础已存在 |
| WCA-P6-001 | Phase 6 | CAD/CAE context source 面板 | TODO | P2 | `useGeometryPointers.ts`, `ContextSummaryPanel` | 显示 Agent 使用的工程上下文 | 依赖 summary |
| WCA-P6-002 | Phase 6 | 仿真任务 Plan 模板 | TODO | P2 | `intent_planner.py`, `prompts.py` | CAE plan 与工具链匹配 | 审批边界明确 |
| WCA-P6-003 | Phase 6 | CAD 修改任务恢复机制 | TODO | P2 | `engine.py`, `context_memory.py` | CAD build error 可修复继续 | 强化既有 repair loop |
