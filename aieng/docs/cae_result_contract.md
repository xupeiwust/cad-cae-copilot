...
## Package-level normalized summary

`results/result_summary.json` is the package-level, LLM/UI-facing normalized
view generated by `aieng.cae_result_summary.generate_cae_result_summary`. It is
not a solver runner and does not create physical evidence; it only normalizes
existing package artifacts into one honest view.

The summary carries a `result_contract` block:

```jsonc
{
  "schema_version": "0.1",
  "canonical_summary_path": "results/result_summary.json",
  "legacy_rest_summary_path": "simulation/results_summary.json",
  "claim_tier": "executed_solver_result | imported_computed_metrics | legacy_rest_result | missing_or_unknown",
...
- `executed_solver_result`: at least one `simulation/runs/*/solver_run.json`
  exists with completed state and `solved=true`.
- `imported_computed_metrics`: normalized metrics exist, but no completed
  solver-run metadata is present. This can support scalar reporting but not a
  claim that AIENG executed the solver.
- `legacy_rest_result`: only the older REST all-in-one
  `simulation/results_summary.json` success artifact is present. It is accepted
  for backwards-compatible scalar display, but it is not MCP
  `cae.run_solver` evidence.
- `missing_or_unknown`: no completed solver-run evidence and no normalized
  metrics are available.

Reader guidance:

- Use `result_contract.claim_tier` and `solver_execution_evidence` for UI badges,
  agent summaries, and audit wording.
- Use `computed_values.source` for the scalar metric source. It may be
  `results/computed_metrics.json` or the legacy compatibility source
  `simulation/results_summary.json`.
- Never infer solver execution merely from numeric metrics. Solver execution is
  proved by completed `solver_run.json` metadata, not by a scalar summary alone.
