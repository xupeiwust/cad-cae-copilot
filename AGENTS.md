...
| Tool | Purpose |
|------|---------|
| `aieng.convert` | Import STEP/FCStd/Shape IR into a `.aieng` package. Shape IR compiles by `representation`: `brep_build123d` (default) → build123d STEP/B-Rep + analytic faces; `implicit_sdf` → fogleman/sdf mesh; `manifold_mesh` → manifold3d CSG mesh (mesh reps give region-level faces). Publishes a viewer preview |
