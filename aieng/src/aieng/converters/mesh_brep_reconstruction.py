...
def plan_partial_brep(
    ...
    face_gen_by_region: dict[str, dict[str, Any]] = {}
    if isinstance(freeform_brep_faces, dict):
        face_gen_by_region = {
        ...
        for region in region_graph["regions"]:
            ...
            if s is None:
                ...
                if ff is not None and ff.get("status") == "fitted":
                    ...
                    # Optional face generation annotation
                    fg = face_gen_by_region.get(rid)
                    if fg:
                        row["freeform_face_candidate_id"] = fg.get("face_id")
