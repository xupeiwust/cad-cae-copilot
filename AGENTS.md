...
| `opt.writeback_to_shape_ir` | Author the optimization result back into `geometry/shape_ir.json`, then recompile through runtime routing → the optimized body meshes/views + gets verification + object_registry, linked to its `design_space_node`. `method=contour` (default) writes a marching-squares boundary as an `extruded_region` (`boundary=spline` default → closed periodic spline / CAD-friendly curve, `boundary=polygon` → straight segments); `method=voxels` writes the blocky `density_voxels`. Placed in the design-space frame from the derivation. Default representation `manifold_mesh`; `brep_build123d` also supported |
